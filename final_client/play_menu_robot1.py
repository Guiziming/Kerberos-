...
def get_data_2013(usrname,d_c,n_c):
        sign_2013 = get_sign(b'2013', d_c, n_c)  # int
        message_2013 = usrname + '|' + str(sign_2013)
        return message_2013
...
def play_menu_robot(client_socket,flag_color,des_c_v,n_c,e_c,d_c,e_v,n_v,IDc,username):
    # 定义时间限制
    time_limit = 30
   ...
   '''
    data13=username
    data_2013 = get_data_2013(data13, d_c, n_c)
    encry_message_2013 = des.encryption(data_2013, des_c_v)  # str
    packet_2013 = packet_head(b'65434', IDc.encode(), b'2013', b'0', b'1004', b'00000000', encry_message_2013.encode())
    print("我已发送自己的玩家信息给服务器!")
    client_socket.sendall(packet_2013)
    print("packet_2013: ",packet_2013)
    packet_2014=client_socket.recv(1024)
    print("packet_2014: ",packet_2014)
    type_message_2014,fin_2014,pipei_2014,message_2014=unpacket(packet_2014) #str
    decry_message_2014 = des.decrypt(message_2014, des_c_v)  # str
    data_2014, sign_2014 = decry_message_2014.split('|')  # str

    data_sign_2014 = recv_sign(int(sign_2014), e_v, n_v)
    user_color='0'
    if b'2014'==data_sign_2014:
        username_14,user_rank,user_color=data_2014.split(',')
        player1_name.insert(tk.END, username_14)
        player1_score.insert(tk.END,user_rank) 
    else:
            ...
            if chess_exist[col][row]==0:
                ...
                if flag_color=='0':
                    ...
                    if game_judge(game_result)==1:
                        ...
                        print("白棋信息",packet_2019_2)
                        white_message=recv_message_2019(packet_2019_2)
                        print("!!!white_message",white_message)
    ...
    # 运行主循环
    root.mainloop()
def robot_game(des_c_v,n_c,e_c,d_c,e_v,n_v,IDc,username):   
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        client_socket.connect((HOST, PORT))
        # type_message='2014'
        # client_socket.sendall(type_message.encode())
        # client_socket.recv(1024) 
        play_menu_robot(client_socket,flag_color,des_c_v,n_c,e_c,d_c,e_v,n_v,IDc,username)   
