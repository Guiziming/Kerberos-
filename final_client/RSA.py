...
def change_int_to_bytes(my_uint):
    if my_uint == 0:
        return bytes(1)
    result = my_uint.to_bytes((my_uint.bit_length() + 7) // 8, 'big')  # 8位划分，尽量少补零
    # str1=str(my_uint)
...
def rsa_encrypt2(str, e, n):
    int_data = change_byte_to_int(str)
    result = fast_mod_exp(int_data, e, n)
    return result
...
def rsa_decrypt2(my_int, d, n):
    int_data = fast_mod_exp(my_int, d, n)
    result = change_int_to_bytes(int_data)
    return result
